...
public class CompleteBinaryTree {
	...
	private void printTreeByPreOrder(int current){
		if(current <= lastIndex){
			System.out.print(nodes[current]+" "); // 현재 노드 방문처리  → 위에동그라미
			printTreeByPreOrder(current*2);// 왼쪽 자식노드 방문처리 → 왼쪽동그라미
			printTreeByPreOrder(current*2+1);// 오른쪽 자식노드 방문처리 → 오른쪽동그라미
		}
	}
	...
	private void printTreeByInOrder(int current){
		if(current <= lastIndex){
			printTreeByInOrder(current*2);			// 왼쪽 자식노드 방문처리 → 왼쪽동그라미
			System.out.print(nodes[current]+" ");		// 현재 노드 방문처리 → 가운데(위) 동그라미
			printTreeByInOrder(current*2+1);			// 오른쪽 자식노드 방문처리 → 오른쪽 동그라미
		}
	}
