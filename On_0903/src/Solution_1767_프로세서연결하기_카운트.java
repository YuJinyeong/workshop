...
public class Solution_1767_프로세서연결하기_카운트 {
	public static int map[][], N, max, min, totalCnt; // 멕시노스판, 판크기, 최대코어수, 최소전선길이, 처리할 코어수
	...
	private static void go(int index, int cCnt, int lCnt) { // index: 처리할 코어의 index, cCnt: 직전까지 포함된 코어수
															// lCnt : 연결된 전선의 길이
		// 현재까지 연결된 코어수 + 앞으로 처리해야할 남은 코어수 : 기대할 수 있는 최대코어수
		// 최대코어수가 임시해(현재까지 구해진 최대값)보다 작다면 진행이 의미 없음
	...
	// 현 코어의 위치에서 해당 방향으로 전선을 놓느 게 가능한지 체크
